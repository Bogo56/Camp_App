...
// Syncronous Code errors are handled be express automatically
// No need for catch()
...
router.get("", AsyncCatch (async (req,res) => {
    ...
    res.render("destinations.ejs",{destinations,clusterMap:true})
}))

router.get("/new",isLoggedin,AsyncCatch( async (req,res) =>{
    res.render("new")
}))
...
router.put("/new",isLoggedin,upload.array("images"),validateDestinations, AsyncCatch( async(req,res) => {
    if(!req.body.formData) throw new ExpressError("Invalid Destination Info", 400)
    ...
    destination.geometry = await mapDestination(req.body.formData.location)
...
router.put("/:id",isDestAuthor,upload.array("images"),validateDestinations,AsyncCatch( async (req,res) =>{
    const id = req.params.id
    if(!req.body.formData) throw new ExpressError(" Invalid Destination Info",400)
    const updateInfo = req.body.formData
    new_images = req.files.map(f => ({url:f.path,filename:f.filename}))
    let destination = await destModel.findByIdAndUpdate(id,updateInfo,{new:true})
    // Deleting images
    if(req.body.imgsToDelete){
       await destModel.findByIdAndUpdate(id,{$pull:{images:{filename:{$in:[req.body.imgsToDelete]}}}})
    }
...
router.delete("/:id",isLoggedin,isDestAuthor,AsyncCatch( async (req,res) => {
    try{
